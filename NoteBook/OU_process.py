...
# Juste like AR(1), however, this model is continuous, and the random variable is a brownian motion model
...
from stationarity_test import rolling_adf
...
def gated_rolling_ou(prices, window=60):
    ...
    for t in range(window, len(prices)):
        # the last window, not inculding t
        window_slice = prices[t-window:t]
